...
import com.hudson.hibernatesynchronizer.editors.synchronizer.PartitionScanner;
			...
			for (Iterator i=regions.iterator(); i.hasNext(); ) {
				...
				if (region.getType().equals(PartitionScanner.XML_TAG)) {
					if (region.getOffset() + region.getLength() > offset) {
						...
						if (index > -1) {
							if (index < offset) {
								...
								if (null != evaluator) if (!evaluator.evaluate(cs.currentNode)) return cs;
								break;
							}
							else {
					...
					if (isNodeHeader(region, doc)) {
						...
						if (null != evaluator) if (!evaluator.evaluate(n)) return cs;
						...
						if (region.getOffset() + region.getLength() > offset) {
							...
							if (null != evaluator) if (!evaluator.evaluate(new Node(parentNode, region, doc))) return cs;
							nodeHierarchy.pop();
						}
					}
