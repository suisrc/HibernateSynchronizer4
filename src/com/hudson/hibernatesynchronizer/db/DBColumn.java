...
//		for (Iterator i=table.getPkColumns().iterator(); i.hasNext(); ) {
//			DBColumn column = (DBColumn) i.next();
//			if (column.getName().equals(name)) primaryKey = true;
//		}
	}
	...
	public String getPropName (boolean checkFK) {
		...
		if (isPrimaryKey() && getTable().getPkColumns().size() == 1) {
			...
			else if (firstLetterUpper) return "Id";
			else return "id";
		}
		if (checkFK && isForeignKey()) return getFKPropName();
		else {
			if (null == propName) {
				if (getName().toUpperCase().equals(getName())) {
					if (!firstLetterUpper)
						propName = HSUtil.firstLetterLower(HSUtil.getJavaNameCap(getName().toLowerCase()));
					else
						propName = HSUtil.firstLetterUpper(HSUtil.getJavaNameCap(getName().toLowerCase()));
				}
				else {
					if (!firstLetterUpper)
						propName = HSUtil.firstLetterLower(HSUtil.getJavaNameCap(getName()));
					else
						propName = HSUtil.firstLetterUpper(HSUtil.getJavaNameCap(getName()));
				}
				if (((propName.toUpperCase().startsWith("IS") && propName.length() > 2 && Character.isUpperCase(propName.toCharArray()[2]))
					...
					if (!firstLetterUpper)
						propName = HSUtil.firstLetterLower(propName.substring(size, propName.length()));
					else
						propName = HSUtil.firstLetterUpper(propName.substring(size, propName.length()));
	...
	private String getFKPropName () {
		...
		if (null == fkPropName) {
			...
			if (propName.endsWith("Id")) {
				if (!firstLetterUpper)
					fkPropName = HSUtil.firstLetterLower(propName.substring(0, propName.length() - 2));
				else
					fkPropName = HSUtil.firstLetterUpper(propName.substring(0, propName.length() - 2));
			}
			else if (propName.endsWith("Oid")) {
				if (!firstLetterUpper)
					fkPropName = HSUtil.firstLetterLower(propName.substring(0, propName.length() - 3));
				else
					fkPropName = HSUtil.firstLetterUpper(propName.substring(0, propName.length() - 3));
			}
			else fkPropName = propName;
		}
		return fkPropName;
	}
