...
public class TableRelationship {
	...
	private String getOneToManyListName () {
		...
		String name = null;
		if (count == 1) {
			name = HSUtil.firstLetterUpper(getListName(childTable.getClassName()));
		}
		else {
			...
			name = HSUtil.firstLetterUpper(getListName(childTable.getClassName())) + "By" + sb.toString();
		}
		String javaName = HSUtil.getJavaName(childTable.getName());
		if (name.startsWith(javaName) && Character.isUpperCase(name.toCharArray()[javaName.length()])) {
			name = HSUtil.firstLetterUpper(getListName(name.substring(javaName.length(), name.length())));
		}
		if (!firstLetterUpper) {
			name = HSUtil.firstLetterLower(name);
		}
		return name;
	}
	...
	private String getManyToManyListName () {
		...
		String name = null;
		if (count == 1) {
			name = HSUtil.firstLetterUpper(getManyToManyTable().getClassName());
		}
		else {
			if (getChildTable().getHibernateClassName().toLowerCase().startsWith(getParentTable().getHibernateClassName().toLowerCase())) {
				name = HSUtil.firstLetterUpper(getChildTable().getHibernateClassName().substring(getParentTable().getHibernateClassName().length(), getChildTable().getHibernateClassName().length()));
			}
			else if (getChildTable().getHibernateClassName().toLowerCase().endsWith(getParentTable().getHibernateClassName().toLowerCase())) {
				name = HSUtil.firstLetterUpper(getChildTable().getHibernateClassName().substring(0, getChildTable().getHibernateClassName().length() - getParentTable().getHibernateClassName().length()));
			}
			else {
				name = HSUtil.firstLetterUpper(getChildTable().getHibernateClassName());
			}
		}
