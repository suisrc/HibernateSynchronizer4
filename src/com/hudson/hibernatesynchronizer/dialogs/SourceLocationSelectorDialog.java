...
public class SourceLocationSelectorDialog extends Dialog {
	...
	private ComboEditor sourceLocations;
	...
	protected Control createDialogArea(Composite parent) {
		...
		sourceLocations = new ComboEditor(container, "SourceLocation");
		...
		sourceLocations.select(selection);
		return container;
	}
	...
	protected void okPressed() {
	    String s = sourceLocations.getSelection();
	    Plugin.setProperty(project.getProject(), Constants.PROP_PROJECT_SOURCE_LOCATION, s);
		for (int i=0; i<potentialRoots.size(); i++) {
